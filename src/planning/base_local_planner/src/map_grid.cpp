...
namespace base_local_planner{
  ...
  inline bool MapGrid::updatePathCell(MapCell* current_cell, MapCell* check_cell,
    ...
    if(! getCell(check_cell->cx, check_cell->cy).within_robot &&
        (cost == UstarCostmap::LETHAL_OBSTACLE ||
         cost == UstarCostmap::INSCRIBED_INFLATED_OBSTACLE ||
         cost == UstarCostmap::NO_INFORMATION)){
  ...
  void MapGrid::setTargetCells(const UstarCostmap::Costmap2D& costmap,
    ...
    for (i = 0; i < adjusted_global_plan.size(); ++i) {
      ...
      if (costmap.worldToMap(g_x, g_y, map_x, map_y) && costmap.getCost(map_x, map_y) != UstarCostmap::NO_INFORMATION) {
  ...
  void MapGrid::setLocalGoal(const UstarCostmap::Costmap2D& costmap,
      const std::vector<geometry_msgs::PoseStamped>& global_plan) {
    sizeCheck(costmap.getSizeInCellsX(), costmap.getSizeInCellsY());
  ...
  void MapGrid::computeTargetDistance(queue<MapCell*>& dist_queue, const UstarCostmap::Costmap2D& costmap){
