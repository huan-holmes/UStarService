...
#include <costmap/utils/testing_helper.h>
...
void testClearBehavior(std::string name, 
                       ...
                       bool obstacles, 
                       bool static_map,
                       UstarCostmap::Costmap2DROS* global_costmap, 
                       UstarCostmap::Costmap2DROS* local_costmap){
...
{
    UstarCostmap::Costmap2DROS global(name + "/global", *transformer);
    UstarCostmap::Costmap2DROS  local(name + "/local" , *transformer);
    boost::shared_ptr<UstarCostmap::ObstacleLayer> olayer;
        
    std::vector<boost::shared_ptr<UstarCostmap::Layer> >* plugins = global.getLayeredCostmap()->getPlugins();
    for (std::vector<boost::shared_ptr<UstarCostmap::Layer> >::iterator pluginp = plugins->begin(); pluginp != plugins->end(); ++pluginp) {
        ...
        if(plugin->getName().find("obstacles")!=std::string::npos){
            olayer = boost::static_pointer_cast<UstarCostmap::ObstacleLayer>(plugin);
