...
void UniversalDrive::CheckData()
{
    ...
    if (FourWheelFlag == packet_[1])
    {
        ...
        if (four_wheel.type)
        {
            info_.vel = (four_wheel.left_front + four_wheel.right_front) * kinematic_.wheel_radius * 2 * M_PI / (2 * kinematic_.slow_down * 60);
    ...
    if (AckerManNormallFlag == packet_[1])
    {
        ...
        {
            info_.vel = (ackerman_normal.back) * kinematic_.wheel_radius * 2 * M_PI / (1 * kinematic_.slow_down * 60);
    ...
    if (DiffFlag == packet_[1])
    {
        ...
        if (diff.type)
        {
            info_.vel = (diff.left + diff.right) * kinematic_.wheel_radius * 2 * M_PI / (2 * kinematic_.slow_down * 60);
            info_.w = (diff.right - diff.left) * kinematic_.wheel_radius * 2 * M_PI / (kinematic_.wheel_gauge * kinematic_.slow_down * 60);
...
UniversalNode::UniversalNode()
{
    ...
    std::cout << "robooster_slow_down " << kinematic_.slow_down << "robooster_wheel_radius " << kinematic_.wheel_radius << "robooster_encoder_lines " << kinematic_.encoder_lines << "robooster_wheel_gauge " << kinematic_.wheel_gauge << std::endl;
    std::cout << "-----------------------------------------------------------------" << std::endl;
...
void UniversalNode::Run()
{
    ...
    while (ros::ok())
    {
        ...
        PublicImu(drive_.imu_msgs_.angular_velocity.z);
        last_mseconds = drive_.info_.stamp;
        loop_rate.sleep();
        ros::spinOnce();
    }
