...
MatchProcessor::TNode::TNode(const OrientedPoint& p, double w, TNode* n, unsigned int c){
	pose=p; 
	weight=w;
	childs=c;
	parent=n;
...
MatchProcessor::TNodeVector MatchProcessor::getTrajectories() const{
  TNodeVector v;
...
void MatchProcessor::integrateScanSequence(MatchProcessor::TNode* node){
	//reverse the list 
	TNode* aux=node;
	TNode* reversed=0;
	double count=0;
...
void  MatchProcessor::updateTreeWeights(bool weightsAlreadyNormalized){
...
double propagateWeight(MatchProcessor::TNode* n, double weight){
	if (!n)
		return weight;
	double w=0;
	n->visitCounter++;
...
double MatchProcessor::propagateWeights(){
  // don't calls this function directly, use updateTreeWeights(..) !
