...
namespace UstarCostmap
{
  ...
  void CostmapLayer::updateWithMax(UstarCostmap::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
  {
    if (!enabled_)
      return;
  ...
  void CostmapLayer::updateWithTrueOverwrite(UstarCostmap::Costmap2D &master_grid, int min_i, int min_j,
                                             int max_i, int max_j)
  {
    if (!enabled_)
      return;
  ...
  void CostmapLayer::updateWithOverwrite(UstarCostmap::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
  {
    if (!enabled_)
      return;
    unsigned char *master = master_grid.getCharMap();
  ...
  void CostmapLayer::updateWithAddition(UstarCostmap::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
  {
    if (!enabled_)
      return;
    unsigned char *master_array = master_grid.getCharMap();
    ...
    for (int j = min_j; j < max_j; j++)
    {
      ...
      for (int i = min_i; i < max_i; i++)
      {
        ...
        else
        {
          ...
          if (sum >= UstarCostmap::INSCRIBED_INFLATED_OBSTACLE)
            master_array[it] = UstarCostmap::INSCRIBED_INFLATED_OBSTACLE - 1;
          else
            master_array[it] = sum;
        }
        it++;
