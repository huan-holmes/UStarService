...
namespace UstarCostmap
{
  ...
  void Costmap2DROS::loadOldParameters(ros::NodeHandle &nh)
  {
    ...
    if (nh.getParam("static_map", flag) && flag)
    {
      ...
      map["type"] = XmlRpc::XmlRpcValue("UstarCostmap::StaticLayer");
    ...
    if (nh.getParam("map_type", s) && s == "voxel")
    {
      ...
      map["type"] = XmlRpc::XmlRpcValue("UstarCostmap::VoxelLayer");
      super_map.setStruct(&map);
      plugins.push_back(super_map);
    ...
    {
      map["name"] = XmlRpc::XmlRpcValue("obstacle_layer");
      map["type"] = XmlRpc::XmlRpcValue("UstarCostmap::ObstacleLayer");
      super_map.setStruct(&map);
      plugins.push_back(super_map);
    }
    ...
    map["type"] = XmlRpc::XmlRpcValue("UstarCostmap::InflationLayer");
    super_map.setStruct(&map);
    plugins.push_back(super_map);
  ...
  void Costmap2DROS::copyParentParameters(const std::string &plugin_name, const std::string &plugin_type, ros::NodeHandle &nh)
  {
    ...
    if (plugin_type == "UstarCostmap::StaticLayer")
    {
    ...
    else if (plugin_type == "UstarCostmap::VoxelLayer")
    {
    ...
    else if (plugin_type == "UstarCostmap::ObstacleLayer")
    {
    ...
    else if (plugin_type == "UstarCostmap::InflationLayer")
    {
  ...
  void Costmap2DROS::reconfigureCB(UstarCostmap::Costmap2DConfig &config, uint32_t level)
  {
    ...
    double map_width_meters = config.width, map_height_meters = config.height, resolution = config.resolution, origin_x = config.origin_x,
  ...
  void Costmap2DROS::readFootprintFromConfig(const UstarCostmap::Costmap2DConfig &new_config,
  ...
  void Costmap2DROS::movementCB(const ros::TimerEvent &event)
  {
    ...
    else
    {
      ...
      robot_stopped_ = (tf2::Vector3(old_pose_.pose.position.x, old_pose_.pose.position.y,
                                     old_pose_.pose.position.z)
                            .distance(tf2::Vector3(new_pose.pose.position.x,
                       ...
                       (tf2::Quaternion(old_pose_.pose.orientation.x,
                                        ...
                                        old_pose_.pose.orientation.w)
                            .angle(tf2::Quaternion(new_pose.pose.orientation.x,
