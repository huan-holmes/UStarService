#include <algorithm>
#include "costmap/layers/inflation_layer.h"
#include "costmap/utils/costmap_math.h"
#include "costmap/utils/footprint.h"
#include <boost/thread.hpp>
#include <pluginlib/class_list_macros.h>

PLUGINLIB_EXPORT_CLASS(UstarCostmap::InflationLayer, UstarCostmap::Layer)

using UstarCostmap::INSCRIBED_INFLATED_OBSTACLE;
...
namespace UstarCostmap
{
  ...
  InflationLayer::InflationLayer(): 
        resolution_(0), inflation_radius_(0), inscribed_radius_(0), weight_(0), 
        inflate_unknown_(false), cell_inflation_radius_(0), cached_cell_inflation_radius_(0), 
  ...
  {
    {
      ...
      dynamic_reconfigure::Server<UstarCostmap::InflationPluginConfig>::CallbackType cb = boost::bind(
      ...
      {
        dsrv_ = new dynamic_reconfigure::Server<UstarCostmap::InflationPluginConfig>(ros::NodeHandle("~/" + name_));
        dsrv_->setCallback(cb);
      }
    }
    matchSize();
  ...
  void InflationLayer::reconfigureCB(UstarCostmap::InflationPluginConfig &config, uint32_t level)
  {
  ...
  void InflationLayer::updateCosts(UstarCostmap::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
  {
    boost::unique_lock<boost::recursive_mutex> lock(*inflation_access_);
