...
#include "costmap/utils/costmap_math.h"
#include <tf2_ros/message_filter.h>

#include <pluginlib/class_list_macros.h>
#include <sensor_msgs/point_cloud2_iterator.h>

PLUGINLIB_EXPORT_CLASS(UstarCostmap::ObstacleLayer, UstarCostmap::Layer)

using UstarCostmap::FREE_SPACE;
using UstarCostmap::LETHAL_OBSTACLE;
using UstarCostmap::NO_INFORMATION;

using UstarCostmap::Observation;
using UstarCostmap::ObservationBuffer;
...
namespace UstarCostmap 
{
  ...
  {
    dsrv_ = new dynamic_reconfigure::Server<UstarCostmap::ObstaclePluginConfig>(nh);
    dynamic_reconfigure::Server<UstarCostmap::ObstaclePluginConfig>::CallbackType cb = boost::bind(
  ...
  void ObstacleLayer::reconfigureCB(UstarCostmap::ObstaclePluginConfig &config, uint32_t level)
  {
  ...
  void ObstacleLayer::updateCosts(UstarCostmap::Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j)
  {
    if (!enabled_)
      return;
    ...
    if (footprint_clearing_enabled_)
    {
      setConvexPolygonCost(transformed_footprint_, UstarCostmap::FREE_SPACE);
    }
  ...
  void ObstacleLayer::addStaticObservation(UstarCostmap::Observation &obs, bool marking, bool clearing)
  {
    if (marking)
