...
#include "localization/sensor/odometrysensor.h"
#include "localization/sensor/lasersensor.h"
#include "localization/utils/portable_utils.hpp"
...
using namespace UstarLocalization;
...
class LocalizationNode
{
  ...
  private:
    ...
    dynamic_reconfigure::Server<localization::LOCALIZATIONConfig> *dsrv_;
    localization::LOCALIZATIONConfig default_config_;
  ...
  //beam skip related params
    ...
    void reconfigureCB(localization::LOCALIZATIONConfig &config, uint32_t level);
...
std::vector<std::pair<int,int> > LocalizationNode::free_space_indices;

#define USAGE "USAGE: amcl"

boost::shared_ptr<LocalizationNode> amcl_node_ptr;
...
int main(int argc, char** argv)
{
  ...
  amcl_node_ptr.reset(new LocalizationNode());
        ...
        first_reconfigure_call_(true)
{
  ...
  dsrv_ = new dynamic_reconfigure::Server<localization::LOCALIZATIONConfig>(ros::NodeHandle("~"));
  dynamic_reconfigure::Server<localization::LOCALIZATIONConfig>::CallbackType cb = boost::bind(&LocalizationNode::reconfigureCB, this, _1, _2);
  ...
  diagnosic_updater_.add("Standard deviation", this, &LocalizationNode::standardDeviationDiagnostics);
}

void LocalizationNode::reconfigureCB(localization::LOCALIZATIONConfig &config, uint32_t level)
{
  ...
  pf_ = pf_alloc(min_particles_, max_particles_,
                 ...
                 (pf_init_model_fn_t)LocalizationNode::uniformPoseGenerator,
                 (void *)map_);
...
void LocalizationNode::runFromBag(const std::string &in_bag_fn, bool trigger_global_localization)
{
  rosbag::Bag bag;
                                     ...
                                     std_srvs::Empty::Response& res)
{
  ...
  pf_init_model(pf_, (pf_init_model_fn_t)LocalizationNode::uniformPoseGenerator,
                (void *)map_);
...
LocalizationNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(lasers_update_[laser_index])
  {
    ...
    lasers_[laser_index]->UpdateSensor(pf_, (SensorData*)&ldata);
 ...
 */
void
LocalizationNode::applyInitialPose()
{
  boost::recursive_mutex::scoped_lock cfl(configuration_mutex_);
...
void
LocalizationNode::standardDeviationDiagnostics(diagnostic_updater::DiagnosticStatusWrapper& diagnostic_status)
{
