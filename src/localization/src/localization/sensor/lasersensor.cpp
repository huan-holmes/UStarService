...
namespace UstarLocalization
{
  ...
  ////////////////////////////////////////////////////////////////////////////////
  // Default constructor
  LaserSensor::LaserSensor(size_t max_beams, map_t *map) : Sensor(),
  ...
  void
  LaserSensor::SetModelBeam(double z_hit,
                            double z_short,
                            double z_max,
                            double z_rand,
                            double sigma_hit,
  ...
  void
  LaserSensor::SetModelLikelihoodField(double z_hit,
                                       double z_rand,
                                       double sigma_hit,
                                       double max_occ_dist)
  {
  ...
  void
  LaserSensor::SetModelLikelihoodFieldProb(double z_hit,
                                           double z_rand,
                                           double sigma_hit,
                                           double max_occ_dist,
                                           bool do_beamskip,
  ...
  bool LaserSensor::UpdateSensor(pf_t *pf, SensorData *data)
  {
    if (this->max_beams < 2)
      return false;
  ...
  double LaserSensor::BeamModel(LaserSensorData *data, pf_sample_set_t *set)
  {
    LaserSensor *self;
    int i, j, step;
    double z, pz;
  ...
  double LaserSensor::LikelihoodFieldModel(LaserSensorData *data, pf_sample_set_t *set)
  {
    LaserSensor *self;
    int i, j, step;
    double z, pz;
  ...
  double LaserSensor::LikelihoodFieldModelProb(LaserSensorData *data, pf_sample_set_t *set)
  {
    LaserSensor *self;
    int i, j, step;
    double z, pz;
  ...
  void LaserSensor::reallocTempData(int new_max_samples, int new_max_obs)
  {
    if (temp_obs)
    {
      for (int k = 0; k < max_samples; k++)
